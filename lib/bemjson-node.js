...
class BemjsonNode {
    ...
    /**
     * Returns an array of BemjsonNode objects a node contains of.
     *
     * @returns {BemjsonNode[]}
     */
    get entities() {
        ...
        const baseEntity = new BemEntityName({
            block: this.data_.block,
            elem: this.data_.elem
        });

        this.entities_.push(baseEntity);

        const mods = this.data_.elem ? this.data_.elemMods : this.data_.mods;
        if (mods) {
            Object.keys(mods).forEach(name => {
                const val = mods[name];
                this.entities_.push(new BemEntityName(Object.assign(baseEntity.toJSON(), {mod: {name, val}})));
            });
        }
